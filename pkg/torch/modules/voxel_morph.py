...
class UnetCore(nn.Module):
    ...
    def forward(self, x):
        ...
        x_enc = [x]
        for l_fn in self.enc:
            x_enc.append(l_fn(x_enc[-1]))
...
class CVPR2018Net(nn.Module):
    ...
    def __init__(self, vol_size, enc_nf, dec_nf, full_size=True):
        ...
        """
        super(CVPR2018Net, self).__init__()

        dim = len(vol_size)

        self.unet_model = UnetCore(dim, enc_nf, dec_nf, full_size)
...
class SpatialTransformer(nn.Module):
    """
    [SpatialTransformer] represents a spatial transformation block
